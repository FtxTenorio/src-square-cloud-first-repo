/**
 * Nexus Logger - Beautiful colored console logs
...
`;
    ...
    nexus: {
        ready: (botName, guildCount) => {
            logger.box(`🔗 NEXUS ONLINE`, `Bot: ${botName}\nServidores: ${guildCount}\nStatus: Operacional`);
        },
        moduleLoaded: (moduleName, count) => {
            logger.success('NEXUS', `📦 Módulo ${colors.brightYellow}${moduleName}${colors.reset} carregado (${count} itens)`);
        },
        shutdown: () => {
            logger.warn('NEXUS', '🔌 Desligando Nexus...');
        }
    },
    ...
    discord: {
        ready: (botName, guildCount) => {
            logger.nexus.ready(botName, guildCount);
        },
        command: (commandName, userName, guildName) => {
            logger.info('CMD', `/${colors.brightYellow}${commandName}${colors.reset} por ${colors.brightCyan}${userName}${colors.reset} em ${colors.gray}${guildName}${colors.reset}`);
        },
        ...
        join: (userName, guildName) => {
            logger.info('EVENT', `👋 ${colors.brightGreen}${userName}${colors.reset} entrou em ${guildName}`);
        },
        leave: (userName, guildName) => {
            logger.info('EVENT', `👋 ${colors.brightRed}${userName}${colors.reset} saiu de ${guildName}`);
        },
        error: (error) => {
            logger.error('DISCORD', `${error.message || error}`);
        }
    },
    ...
    db: {
        connected: (dbName) => {
            logger.success('MONGO', `🍃 Conectado: ${colors.brightGreen}${dbName}${colors.reset}`);
        },
        disconnected: () => {
            logger.warn('MONGO', '🍃 Desconectado');
        },
        error: (error) => {
            logger.error('MONGO', `${error.message || error}`);
        },
    ...
    redis: {
        ...
        error: (error) => {
            logger.error('REDIS', `${error.message || error}`);
        }
    },
     ...
     */
    ai: {
        request: (userId, provider) => {
            logger.debug('AI', `🤖 Request de ${colors.cyan}${userId}${colors.reset} via ${provider}`);
        },
        response: (chars, time) => {
            logger.debug('AI', `✨ Resposta (${chars} chars, ${time}ms)`);
        },
        error: (error) => {
            logger.error('AI', `${error.message || error}`);
        },
        fallback: () => {
            logger.debug('AI', '🔄 Usando resposta local (fallback)');
        }
    },
    ...
    level: {
        xpGain: (userName, xp, total) => {
            logger.debug('LEVEL', `⭐ ${colors.brightYellow}${userName}${colors.reset} +${xp}XP (${total})`);
        },
        levelUp: (userName, newLevel) => {
            logger.info('LEVEL', `🎉 ${colors.brightYellow}${userName}${colors.reset} → Nível ${colors.brightGreen}${newLevel}${colors.reset}`);
        },
    ...
    http: {
        ...
        listen: (port) => {
            logger.success('HTTP', `🌐 Servidor rodando na porta ${colors.brightGreen}${port}${colors.reset}`);
        }
    },
    ...
    system: {
        start: () => {
            logger.info('SYSTEM', '🚀 Iniciando Nexus...');
        },
        ready: () => {
            logger.success('SYSTEM', '✨ Sistema pronto!');
        },
        shutdown: () => {
            logger.warn('SYSTEM', '🛑 Encerrando...');
        }
    },
